...
def get_movies_content(text, tokens, tokens_verb_noun):
    movies_bag_of_words_eng = ['action', 'adventure', 'animation', 'children', 'comedy', 'crime',
                               'documentary', 'drama', 'fantasy', 'horror', 'musical', 'mystery', 'romance', 'sci-fi', 'war', 'western']
    movies_bag_of_words_ar = ['اكشن', 'مغامره', 'انيميشن', 'اطفال', 'كوميدي', 'جريمه',
                              'وثايقي', 'دراما', 'خيال', 'رعب', 'موسيقي', 'غموض', 'رومانسي', 'خيال', 'حروب', 'ويستيرن']
    categories = []
    ...
    for index, word in enumerate(tokens):
        if tokens_verb_noun[index][1] == 'n':
            index, distance = get_closest_word_with_threshold(
                word, movies_bag_of_words_ar, 0.4)
            if index != -1:
                categories.append(
                    [movies_bag_of_words_eng[index], 1 - distance])
