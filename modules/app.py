...
def get_response():
    ...
    intent, emotion, response = main.main(text, location, stopwords, ner_instance, verbs, nouns, emotions_model,
                                          emotions_tf_idf, intent_model, tokenizer, recomm_intent_model, recomm_tokenizer, q_not_model, tf_idf_q_not)
    print("Intent -> ", intent)
    print("Emotion ->", emotion)
    if intent == 'general':
        messages = user['messages']
        messages = [{"message": message['message'], 'isUser':message['isUser']}
                    for message in messages if message['isGeneral'] == 1]
        if len(messages) > 4:
            messages = messages[:4]
        messages = messages[::-1]
        response = requests.post(
            GENERATION_API, json={'utter': text, 'history': messages})
        response = response.json()
        response['text'] = response['response']

    elif intent == 'recommendation-movies':
        response = movies_recommendation(
            user, response['movie'], response['categories'], "")

    elif intent == 'recommendation-places':
        response = locations_recommendation(
            user, response['places'], "", location)
...
def add_conversation(user, message, id, intent, removeFirst=0):
    ...
    ), 'isGeneral': (1 if intent == "general" else 0), 'intent': intent}
...
def movies_recommendation(user, movie, categories, text):
    ...
    if movie == "" and len(categories) == 0:
        ...
        if len(categories_liked_filtered) == 0:
            categories_liked = [category['name']
                                for category in categories_liked]
            categories, relevance = get_relevance(categories_liked)
            movies = movie_recomm.recommend_given_categories(
                categories, relevance, top_k=50)
...
def locations_recommendation(user, preprocessed_text, text, location):
    ...
    location_data = location_recomm.recommend(
        location_tuple, keyword=preprocessed_text)
    locations = list()
