...
def get_closest_number(word):
    ...
    if word.isnumeric():
        return int(word), 0
    number_index, editDistance = get_closest_word_with_threshold(
        word, numbersBagOfWords)
    return number_index + 1, editDistance
# ------------------------------------------------------------------------


def get_closest_month(word):
    monthsBagOfWords = ["يناير", "فبراير", "مارس", "ابريل", "مايو",
...
def get_closest_word_with_threshold(word, bag_of_words, threshold=0.35):
    ...
    for index, token in enumerate(bag_of_words):
        editDistanceRatio = edit_distance(
            word, token) / min(len(token), len(word))
...
def numbers_bag_of_words():
    singleNumbers = ["واحد", "اتنان", "تلاته", "اربعه",
                     "خمسه", "سته", "سبعه", "تمانيه", "تسعه"]
...
def get_time(tokens, task):
    ...
    bagOfWordsFunctions = [get_closest_hour,
                           get_closest_day, get_closest_month]
        ...
        if gram[0] in time_specifiers_set:
            ...
            if edits[specifier[0]] == 0 and tokens_used[tokens.index(gram[0])] == 0 and tokens_used[tokens.index(gram[1])] == 0:
                ...
                if edits[specifier[0]] != 0:  # "شهر 12","يوم 22","الساعة 4"
                    ...
                    edits[specifier[0]
                          ] = bagOfWordsFunctions[specifier[0] - 1](gram[1])
        ...
        if gram[1] in time_specifiers_increment:
            ...
            if edits[specifier[0]] == 0 and tokens_used[tokens.index(gram[1])] == 0:
                ...
                if specifier[1] == 1 or specifier[1] == 7 and tokens_used[tokens.index(gram[0])] == 0:
                    ...
                    if number > 10:
                        edits[specifier[0]] = specifier[1] * \
                            number  # mode to handle weeks (7 * days)
                        tokens_used[tokens.index(gram[0])] = 1
                    else:
                        edits[specifier[0]] = 0

                # ايام شهور اسابيع ساعات
                elif specifier[1] >= 3 and specifier[1] <= 9 and tokens_used[tokens.index(gram[0])] == 0:
                    number, _ = get_closest_number(gram[0])
                    # mode to handle weeks (7 * days)
                    edits[specifier[0]] = (specifier[1] - 2) * number
                    tokens_used[tokens.index(gram[0])] = 1
                tokens_used[tokens.index(gram[1])] = 1
    # no specifiers
...
def edit_time(edits):
    ...
    new_time = datetime(year=now_time.year, month=(edits[3] * edits[6]) + (now_time.month * (not edits[6])), day=(edits[2] * edits[5]) + (now_time.day * (
        not edits[5])), hour=(edits[1] + 12 * edits[7]) * edits[4] + (now_time.hour * (not edits[4])), minute=(edits[0] * edits[4]) + (now_time.minute * (not edits[4])))
    # add the incremental values
    new_time = new_time + relativedelta(minutes=edits[0] * (not edits[4]), hours=edits[1] * (
        not edits[4]), days=edits[2] * (not edits[5]), months=edits[3] * (not edits[6]))
    ...
    if new_time <= now_time:
        new_time = new_time - \
            relativedelta(minutes=1) - relativedelta(hours=1) + \
            relativedelta(days=1)
    if new_time <= now_time:
        new_time = new_time - relativedelta(minutes=1) - relativedelta(
            hours=1) - relativedelta(days=1) + relativedelta(months=1)
    if new_time <= now_time:
        new_time = new_time - relativedelta(minutes=1) - relativedelta(
            hours=1) - relativedelta(days=1) - relativedelta(months=1) + relativedelta(years=1)
    return new_time
