verb_prefix = {'فسن', 'فست', 'فسي', 'فا', 'سي', 'ست', 'سن',
               'سا', 'فن', 'فت', 'في', 'فس', 'س', 'ف', 'ن', 'ت', 'ي', 'ا', ''}
verb_suffix = {'لي', 'ي', 'تما', 'ا', 'ت', 'ين', 'ان',
               'و', 'وا', 'تن', 'تم', 'تا', 'نا', 'ون', 'ن', ''}
        ...
        for prefix in verb_prefix:
            for suffix in verb_suffix:
                if len(token) - len(prefix) - len(suffix) > 2 and token[: len(prefix)] == prefix and token[len(token) - len(suffix):] == suffix:
                    token_possible_stem = token[len(
                        prefix): len(token) - len(suffix)]
